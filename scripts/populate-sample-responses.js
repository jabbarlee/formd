/**
 * Populate Sample Responses Script
 * Generates realistic sample responses for a specific form using proper schema
 */
    ...
    for (let i = 0; i < numResponses; i++) {
      ...
      const { data: response, error: responseError } = await supabase
        ...
        .insert({
          ...
          ai_sentiment: status === 'completed' ? randomElement(sentimentTypes) : null,
          ai_sentiment_score: status === 'completed' ? randomFloat(0.1, 1.0) : null,
        })
        .select()
        .single();
        ...
        for (const question of questions) {
          ...
          const { error: answerError } = await supabase
            ...
            .insert({
              ...
              ai_sentiment: randomElement(['positive', 'neutral', 'negative']),
              ai_sentiment_score: randomFloat(0.1, 1.0),
              ai_themes: [randomElement(['satisfaction', 'feedback', 'improvement', 'quality'])],
              ai_categories: [randomElement(['general', 'specific', 'suggestion', 'complaint'])]
            });
